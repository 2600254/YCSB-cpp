...
std::vector<rocksdb::ColumnFamilyHandle *> ElasticLSMDB::cf_handles_;
rocksdb::DB *ElasticLSMDB::db_ = nullptr;
int ElasticLSMDB::ref_cnt_ = 0;
std::mutex ElasticLSMDB::mu_;

void ElasticLSMDB::Init() {
// merge operator disabled by default due to link error
  ...
  if (format == "single") {
    format_ = kSingleRow;
    method_read_ = &ElasticLSMDB::ReadSingle;
    method_scan_ = &ElasticLSMDB::ScanSingle;
    method_update_ = &ElasticLSMDB::UpdateSingle;
    method_insert_ = &ElasticLSMDB::InsertSingle;
    method_delete_ = &ElasticLSMDB::DeleteSingle;
#ifdef USE_MERGEUPDATE
    if (props.GetProperty(PROP_MERGEUPDATE, PROP_MERGEUPDATE_DEFAULT) == "true") {
      method_update_ = &ElasticLSMDB::MergeSingle;
    }
#endif
  } else {
...
void ElasticLSMDB::GetOptions(const utils::Properties &props, rocksdb::Options *opt,
                           std::vector<rocksdb::ColumnFamilyDescriptor> *cf_descs) {
...
void ElasticLSMDB::SerializeRow(const std::vector<Field> &values, std::string &data) {
...
void ElasticLSMDB::DeserializeRowFilter(std::vector<Field> &values, const char *p, const char *lim,
...
void ElasticLSMDB::DeserializeRowFilter(std::vector<Field> &values, const std::string &data,
                                     const std::vector<std::string> &fields) {
...
void ElasticLSMDB::DeserializeRow(std::vector<Field> &values, const char *p, const char *lim) {
  while (p != lim) {
    assert(p < lim);
    uint32_t len = *reinterpret_cast<const uint32_t *>(p);
    p += sizeof(uint32_t);
...
void ElasticLSMDB::DeserializeRow(std::vector<Field> &values, const std::string &data) {
  const char *p = data.data();
  const char *lim = p + data.size();
  DeserializeRow(values, p, lim);
}

DB::Status ElasticLSMDB::ReadSingle(const std::string &table, const std::string &key,
                                 const std::vector<std::string> *fields,
...
DB::Status ElasticLSMDB::ScanSingle(const std::string &table, const std::string &key, int len,
                                 const std::vector<std::string> *fields,
...
DB::Status ElasticLSMDB::UpdateSingle(const std::string &table, const std::string &key,
                                   std::vector<Field> &values) {
  std::string data;
  rocksdb::Status s = db_->Get(rocksdb::ReadOptions(), key, &data);
  if (s.IsNotFound()) {
...
DB::Status ElasticLSMDB::MergeSingle(const std::string &table, const std::string &key,
                                  std::vector<Field> &values) {
  std::string data;
...
DB::Status ElasticLSMDB::InsertSingle(const std::string &table, const std::string &key,
                                   std::vector<Field> &values) {
  std::string data;
  SerializeRow(values, data);
  rocksdb::WriteOptions wopt;
...
DB::Status ElasticLSMDB::DeleteSingle(const std::string &table, const std::string &key) {
  rocksdb::WriteOptions wopt;
...
DB *NewElasticLSMDB() {
  return new ElasticLSMDB;
}

const bool registered = DBFactory::RegisterDB("rocksdb", NewElasticLSMDB);
